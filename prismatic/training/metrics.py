...
class VLAMetrics:
    def __init__(
        ...
        self.state = {
            ...
            "output_accuracy": deque(maxlen=window_size),
            "step_time": deque(maxlen=window_size),
            "lr": [],
        }
    ...
    def push(self) -> str:
        ...
        output_accuracy = torch.stack(list(self.state["output_accuracy"])).mean().item()
        step_time, lr = np.mean(list(self.state["step_time"])), self.state["lr"][-1]
        status = self.get_status(loss)
        ...
        self.log(
            ...
            metrics={
                ...
                f"{prefix}/Non-action Output Token Accuracy": output_accuracy,
                f"{prefix}/Loss (Raw)": loss_raw,
                f"{prefix}/Learning Rate": lr,
                f"{prefix}/Step Time": step_time,
                **dataset_metrics,
