    ...
    def run_vla_training(
        ...
        with tqdm(
            ...
            for batch in dataloader:
                ...
                preds = output.logits[:, self.vlm.vision_backbone.num_patches : -1].argmax(dim=2)
                gt = batch["labels"][:, 1:].to(preds.device)
                if fast_first_token >= 0:
                    mask = gt >= fast_first_token
                else:
                    mask = (action_tokenizer.action_token_end_idx > gt) & (gt > action_tokenizer.action_token_begin_idx)

                # Compute Accuracy
                correct_preds = (preds == gt) & mask
                action_accuracy = correct_preds.sum().float() / mask.sum().float()

                # Compute L1 Loss on Predicted (Continuous) Actions
                continuous_actions_pred = torch.tensor(
                    action_tokenizer.decode_token_ids_to_actions(preds[mask].cpu().numpy())
                )
                continuous_actions_gt = torch.tensor(
                    action_tokenizer.decode_token_ids_to_actions(gt[mask].cpu().numpy())
                )
                action_l1_loss = torch.nn.functional.l1_loss(continuous_actions_pred, continuous_actions_gt)

                # gt < 0 means that token is not used for loss (e.g., it is in the input prompt)
                # Get mask that is for all non-action, non-ignore tokens
                non_action_mask = ~mask & (gt >= 0)

                if non_action_mask.sum() > 0:
                    correct_output_preds = (preds == gt) & non_action_mask
                    output_accuracy = correct_output_preds.sum().float() / non_action_mask.sum().float()
                else:
                        ...
                        for ds in datasets:
                            ...
                            continuous_actions_pred_ds = torch.tensor(
                                action_tokenizer.decode_token_ids_to_actions(
                                    preds[ds_mask][mask[ds_mask]].cpu().numpy()
                                )
                            )
