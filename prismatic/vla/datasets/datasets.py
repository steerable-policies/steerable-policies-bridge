...
IGNORE_INDEX = -100
  ...
  # TODO =>> Update with actual path to compiled reasoning data
...
class RLDSBatchTransform:
    ...
    fast_tokenizer: AutoProcessor = None
    ...
    def __call__(self, rlds_batch: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # get the possible subtask level commands for this subtask (or use task label otherwise)
        try:
            # get the subtask for this step
            subtask = step_to_subtask[traj_idx][time_idx]

            # get the possible commands for this subtask
        ...
        if self.fast_tokenizer is not None:
            assert len(action.shape) >= 2, "Action shape must be at least 2D for fast tokenizer"
            if len(action.shape) == 2:
                # Add batch dimension
                action = action.reshape(1, *action.shape)
            action = self.fast_tokenizer(action)
            action = action[0] # remove batch dimension

            tokenized_action = " ".join([f"<|extra_{i}|>" for i in action])
...
@dataclass
class ReasonerRLDSBatchTransform:
    action_tokenizer: ActionTokenizer
    base_tokenizer: PreTrainedTokenizerBase
    image_transform: ImageTransform
    prompt_builder_fn: Type[PromptBuilder]
    ...
    def __call__(self, rlds_batch: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # get the possible subtask level commands for this subtask
        try:
            # get the subtask for this step
            subtask = step_to_subtask[traj_idx][time_idx]
        ...
        # get the rationale
        try:
            rationale = rationales[traj_idx][subtask].strip()
            rationale += "." if rationale[-1] != "." else ""
        except:
            rationale = None
        ...
        num_answer_tokens = len(self.base_tokenizer(output)["input_ids"])
