...
class TrainConfig:
    ...
    use_fast_tokenizer: bool = False                                # Not implemented yet!
...
def train(cfg: TrainConfig) -> None:
    ...
    if cfg.pretrained_checkpoint is not None:
        ...
        vlm = load_vla(
            ...
            num_extra_tokens=2048 if cfg.use_fast_tokenizer else -1
        )
    ...
    if cfg.use_fast_tokenizer:
        assert cfg.future_action_window_size > 0, "Fast Tokenizer requires future action window size > 0"
        overwatch.info(f"Loading fast tokenizer for VLA dataset")
        fast_tokenizer = AutoProcessor.from_pretrained("physical-intelligence/fast", trust_remote_code=True)

        # Only need to add extra tokens and resize embeddings for LLaMa2 if not using a pretrained checkpoint
        if cfg.pretrained_checkpoint is None:
            if type(vlm.llm_backbone) is LLaMa2LLMBackbone:
                # If using LLaMa2, add extra tokens to tokenizer and resize embeddings AFTER
                # weights have been loaded.
                num_extra_tokens = 2048
                added = vlm.llm_backbone.tokenizer.add_tokens([f"<|extra_{i}|>" for i in range(num_extra_tokens)])
                assert added == num_extra_tokens, f"Added {added} of {num_extra_tokens} extra tokens to tokenizer!"
                vlm.llm_backbone.llm.resize_token_embeddings(len(vlm.llm_backbone.tokenizer), pad_to_multiple_of=64)
            else:
                raise NotImplementedError(f"Fast Tokenizer not implemented for {type(vlm.llm_backbone)}")
    ...
    vla_dataset, action_tokenizer, collator = get_vla_dataset_and_collator(
        ...
        train_reasoner=cfg.train_reasoner
    )
    ...
    train_strategy.run_vla_training(
        ...
        fast_first_token=-1 if not cfg.use_fast_tokenizer 
            else vlm.llm_backbone.tokenizer.get_vocab()["<|extra_0|>"]
    )
